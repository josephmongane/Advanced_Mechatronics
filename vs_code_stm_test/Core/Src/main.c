...
int main(void)
{
  ...
  while (1) {
    ...
    while (rx_index < sizeof(rx_message) - 1) {
      HAL_StatusTypeDef status = HAL_UART_Receive(&huart1, &byte, 1, 1000);
      if (status == HAL_OK) {
        if (byte == '\n') {
        ...
        printf("Hal timed out \n");
        break;
      }
      else if (status == HAL_ERROR) {
        printf("Hal timed Error \n");
        break;
      }
      else {
    ...
    // Use HAL transmit directly instead of printf
    printf("Received: %s\n", rx_message);   
